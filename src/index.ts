...
type Op='repeat'|'inc'|'dec'|'normal'|'large';
...
export function compress(values: number[]): Uint8Array{
...
export function compressBytes(bytes: Uint8Array): Uint8Array{
  const vals:number[]=[bytes[0]];
  for(let i=1;i<bytes.length;i++) vals.push(bytes[i]-bytes[i-1]+255);
  return compress(vals);
}
export function decompressBytes(data: Uint8Array): Uint8Array{
  const vals=decompress(data);
  if(vals.length===0) return new Uint8Array();
  const out=new Uint8Array(vals.length);
  out[0]=vals[0];
  for(let i=1;i<vals.length;i++) out[i]=(out[i-1]+vals[i]-255)&0xFF;
  return out;
}
